...
document.addEventListener('DOMContentLoaded', () => {
  getDashboardData() 
    .then((data) => {
      const activities = data.map(e => new DashboardItem(e));
    
      const selectors = document.querySelectorAll('.date-list__item')
      selectors.forEach(selector => selector.addEventListener('click', function() {
        selectors.forEach(sel => sel.classList.remove('date-list__item--active'))
        selector.classList.add('date-list__item--active')

        const currentView = selector.innerText.trim().toLowerCase();
        activities.forEach(e => e.changeView(currentView))
      }))
    })
})